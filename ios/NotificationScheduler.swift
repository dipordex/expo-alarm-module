...
class NotificationScheduler: NotificationSchedulerDelegate {
    ...
    func startManualAlarmTrigger(at date: Date, soundName: String, localPath: String?, uid: String, id: String) {
            ...
            while self.pollingThreads[pollingKey] == true {
                ...
                if now >= date {
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.playAlarmSound(soundName: soundName, localPath: localPath, uid: uid, date: date, id: id)
                        self.stopPolling(for: pollingKey)
                    }
                    break
                }
    ...
    func playAlarmSound(soundName: String, localPath: String?, uid: String, date: Date, id: String) {
        os_log("SetInc_Log: 🔊 Playing alarm sound for UID: %{public}@", log: log, type: .error, uid)
        let alarm =  Store.shared.alarms.getAlarm(ByUUIDStr: uid)
        let formatter = DateFormatter()
        formatter.dateFormat = "HH:mm"
    ...
    func restorePollingThreadsFromScheduledNotifications() {
        ...
        UNUserNotificationCenter.current().getPendingNotificationRequests { requests in
            for request in requests {
                ...
                DispatchQueue.main.async {
                    self.startManualAlarmTrigger(at: fireDate, soundName: soundName, localPath: localPath, uid: uid, id: id)
                }
            }
        }
    }
